...
export default function CodeTabs() {
  ...
  return (
    <div className={styles.installTabs} data-testid="react-code-tabs">
      <div className={styles.toolbar}>
        <div className={styles.list} role="tablist" aria-label="Package manager command switcher">
          {commandTabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
        ...
        <button
          type="button"
          className={`${styles.copy} ${copyStatus === 'copied' ? styles.copyCopied : ''}`}
      ...
      <span className={styles.sr} aria-live="polite">{statusMessage}</span>
    </div>
  );
}
