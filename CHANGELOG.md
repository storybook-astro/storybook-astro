...
- `Date` args also survive the **static build** (`renderMode: 'static'`) prerender path. There the args arrive as real `Date` objects (not transported as strings), and `processImageMetadata` walked them with `Object.entries`, flattening a `Date` to `{}` and producing `RangeError: Invalid time value` when a component formatted it. It now only recurses into plain objects.
