...
### Fixed
- Importing a story whose component uses `astro:assets` failed at module load with `TypeError: Unknown file extension ".astro"` on Vitest below 4.1.0 (#117). `vitest` is now declared as an optional peer dependency (`^4.1.0`) so a mismatched version surfaces a clear warning instead of a cryptic runtime error.
